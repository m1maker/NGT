...
int WriteConfigToStream(asIScriptEngine* engine, ostream& strm)
{
	...
	for (n = 0; n < asEP_LAST_PROPERTY; n++)
		strm << "const int EP_" << n << " = " << engine->GetEngineProperty(asEEngineProp(n)) << ";\n";
	...
	for (n = 0; n < c; n++)
	{
		...
		else
		{
			...
			strm << "class " << engine->GetTypeDeclaration(type->GetTypeId()) << ";    // " << (unsigned int)(type->GetFlags() & asOBJ_MASK_VALID_FLAGS) << "\n";
		...
		static void Write(asIScriptEngine* engine, ostream& strm, asITypeInfo* type, string& currNamespace, asDWORD& currAccessMask)
		{
			...
			else
			{
				...
				for (m = 0; m < type->GetFactoryCount(); m++)
				{
					...
					strm << "obj_beh " << typeDecl.c_str() << " " << asBEHAVE_FACTORY << " " << Escape::Quotes(func->GetDeclaration(false)).c_str() << ";\n";
				}
				for (m = 0; m < type->GetBehaviourCount(); m++)
				{
					...
					if (beh == asBEHAVE_CONSTRUCT)
						// Prefix 'void'
						strm << "obj_beh \"" << typeDecl.c_str() << "\" " << beh << " \"void " << Escape::Quotes(func->GetDeclaration(false)).c_str() << "\"\n";
