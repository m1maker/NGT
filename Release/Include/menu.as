#include "hip.as"
#include "sound_pool.as"

funcdef void menu_item_callback();
funcdef void menu_on_click_callback(menu@);
...
class menu {
	...
	void navigate(int direction) {
		...
		if (selected_index < 0) {
			...
			if (wrapp && wrapp_sound != "")
				sp.play_stationary(wrapp_sound, false); else if (border_sound != "") sp.play_stationary(border_sound, false);
	...
	void monitor() {
		...
		if (key_repeat(KEY_DOWN) || (enable_tab_order && !shift_down && key_repeat(KEY_TAB))) {
			...
			this.navigate(items.length() - 1);
		} else if (key_repeat(KEY_RETURN) || key_repeat(KEY_SPACE) || (key_repeat(KEY_RIGHT) && items[selected_index].submenu !is null)) this.select(); else if (this.in_submenu && (key_repeat(KEY_ESCAPE) || key_repeat(KEY_BACK) || key_repeat(KEY_LEFT))) {
				if (close_sound != "")
	...
	int ampersandPos = inputStr.find_first("&", 0);
		...
		if (ampersandPos != -1 && ampersandPos < inputStr.
	length() - 1)
		{
			// Return the character after the ampersand
			pos = ampersandPos;
			return inputStr.
	substr(ampersandPos + 1, 1);
		}
