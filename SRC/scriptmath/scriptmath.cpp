...
void RegisterScriptMath_Native(asIScriptEngine* engine)
{
	...
	r = engine->RegisterGlobalFunction("double fp_from_ieee(uint64)", asFUNCTIONPR(fpFromIEEE, (asQWORD), double), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterGlobalFunction("uint64 fp_to_ieee(double)", asFUNCTIONPR(fpToIEEE, (double), asQWORD), asCALL_CDECL); assert(r >= 0);
